...
Queries are assembled using the
[elasticsearch Query DSL python client library](https://elasticsearch-dsl.readthedocs.io/en/latest/index.html).

Core query building blocks against the elasticsearch index reside in the `elastic.py` module.

These building blocks are used in the `collections.py`, `learning_material.py` and `stats.py` modules to build
concrete queries against the elasticsearch index. They are also used in some other places across the application.
