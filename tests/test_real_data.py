...
@pytest.mark.skipif(
    "CI" in os.environ,
    reason="This tests needs a running instance of the service and elastic connection",
)
def test_real_data():
    """
    ...
    """
    try:
        httpx.get(url="http://localhost:8081/_ping")
    ...
    for collection, node_id in COLLECTION_NAME_TO_ID.items():
        ...
        response = httpx.get(
            url=f"http://localhost:8081/collections/{node_id}/material-validation"
        )
        ...
        combined = {
            name: union(*(set(getattr(v, name)) for v in validation))
            for name, attribute in attributes.items()
        }

        for name, attribute in attributes.items():
            response = httpx.get(
                url=f"http://localhost:8081/collections/{node_id}/pending-materials/{attribute.value}"
            )
            ...
            if len(combined[name]) != len(pending):
                logger.warning(
                    f"MISMATCH in {collection}/{name}: Combined: {len(combined[name])} <> {len(pending)}"
                )
